...
class View:
    ...
    def setup_player(self):
        try:
            ...
            if self.my_player.get_db_status():
                messagebox.showinfo("Success", "Connect successfully to the DB !")
            else:
                raise Exception("Sorry! You cannot save or load fauvorites!!")
        ...
        img = tk.PhotoImage(file="./icons/icon_music.png")
        self.top.iconphoto(self.top, img)
        self.top.title("MOUZIKKA-DANCE OF HEART...!")
        self.top.protocol("WM_DELETE_WINDOW", self.closewindow)
        self.isPaused = False
        self.isPlaying = False
        self.previousButton.bind("<Button-1>",lambda e,url=self:self.load_previous_song())
        self.Button9.configure(command=self.add_song_to_favourites)
        self.Button10.configure(command=self.remove_song_from_favorites)
    ...
    def load_previous_song(self):
        try:
            ...
            prev_song_index=self.sel_song_index_tuple[0]-1
    ...
    def add_song_to_favourites(self):
        ...
        try:
            if len(song_index)==0:
                raise NoSongSelectedError("Please select the song to add favorites")
            song_name=self.playList.get(song_index[0])
    ...
    def load_songs_from_favourites(self):
        try:
            load_res=self.my_player.load_songs_from_favourites()
            res=load_res[0]
            if res.find("No songs present")!=-1:
                messagebox.showinfo("Favorites Empty!","No songs in your favorites")
                return
            song_dict=load_res[1]
            ...
            print(mycolor)
            self.playList.configure(fg=mycolor)
            messagebox.showinfo("Success!","List Populated from your favorites")
        except DatabaseError as ex2:
            messagebox.showerror("DB Error!","Sorry! song cannot be loaded from favorites")
    ...
    def remove_song_from_favorites(self):
        ...
        try:
            if len(song_index) == 0:
                raise NoSongSelectedError("Please select the song to remove  favorites")
            song_name = self.playList.get(song_index[0])
            result = self.my_player.remove_song_from_favorites(song_name)
            messagebox.showinfo("Success!!", result)
            self.remove_song()
